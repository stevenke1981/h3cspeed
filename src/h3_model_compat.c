...
#define H3_MIN_ADALN_CURVE_GRID UINT64_C(2)
#define H3_ADALN_CURVE_INPUT_DIM UINT64_C(8)
...
h3cspeed_h3_compatibility h3cspeed_h3_model_compatibility(
    ...
    if (config->variant != H3CSPEED_H3_VARIANT_TIME_EMBEDDER &&
        config->variant != H3CSPEED_H3_VARIANT_ADALN_CURVES)
    ...
    } else if (config->variant == H3CSPEED_H3_VARIANT_ADALN_CURVES &&
               (config->adaln_curve_grid < H3_MIN_ADALN_CURVE_GRID ||
                config->time_embed_dim != H3_ADALN_CURVE_INPUT_DIM)) {
        result |= H3CSPEED_H3_INCOMPAT_ADALN_CURVE;
    }
...
int h3cspeed_h3_model_format_compatibility(
    ...
    if (mask & H3CSPEED_H3_INCOMPAT_ADALN_CURVE) {
        if (!append_text(output, output_size, &used,
                         "%sadaln_curve_grid=%llu and curve_dim=%llu (expected grid >= 2 and dim 8)",
                         used ? "; " : "",
                         (unsigned long long)config->adaln_curve_grid,
                         (unsigned long long)config->time_embed_dim)) return 0;
    }
    return 1;
}
