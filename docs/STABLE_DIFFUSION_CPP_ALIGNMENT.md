...
| 模型變體 | time embedder / AdaLN curve table | 兩者皆辨識；目前 CUDA schedule 兩者皆接受 |
...
- variant：`time-embedder` 或 `adaln-curves`
- hidden size：`5376`
- DiT blocks：`50`
...
- text dim：`5120`
- time-embedder：timestep input `256`，time hidden / output `5376 / 2688`
- AdaLN curves：curve table `[grid, 8]`，`grid >= 2`
- RoPE inverse-frequency length：`16`

AdaLN curve-table checkpoint 會被正確辨識，並交由目前 pinned
`h3_dit_schedule.c` 的 curve-table 路徑執行；預檢要求 curve table 為
`[grid, 8]` 且 `grid >= 2`。未知變體或不完整 curve table 仍會 fail-closed。
