...
def standard_tensors(
    *,
    ...
    if adaln:
        tensors[qualified(prefix, "adaln_t_table")] = ("TEST", [24, 2688])
    else:
...
class ModelInfoTests(unittest.TestCase):
    ...
    def test_adaln_variant_is_reported_and_strict_mode_fails(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            component = Path(temporary) / "transformer"
            write_safetensors(
                component / "model.safetensors",
            ...
            self.assertFalse(detected["h3cspeed"]["compatible"])
            self.assertIn("variant=adaln-curves", detected["h3cspeed"]["issues"][0])
