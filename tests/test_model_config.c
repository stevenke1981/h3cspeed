...
static void make_fixture(
    ...
    if (adaln) {
        add(f, prefix, "adaln_t_table", 2, 1025, 8);
    } else {
...
static int adaln_is_detected_and_compatible(void) {
    fixture f;
    h3cspeed_h3_model_config c;
    char error[256] = {0};
    char report[256] = {0};
    ...
    CHECK(c.adaln_curve_grid == 1025 && c.time_embed_dim == 8);
    CHECK(h3cspeed_h3_model_compatibility(&c) == 0);
    CHECK(h3cspeed_h3_model_format_compatibility(&c, report, sizeof(report)));
    CHECK(strstr(report, "compatible") != NULL);
    c.adaln_curve_grid = 1;
    CHECK(h3cspeed_h3_model_compatibility(&c) & H3CSPEED_H3_INCOMPAT_ADALN_CURVE);
    return 1;
}
...
int main(void) {
    ...
    passed += adaln_is_detected_and_compatible();
