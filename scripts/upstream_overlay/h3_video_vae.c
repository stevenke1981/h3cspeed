...
/* Layer-major tiled decode, opt-in with H3_VAE_LAYER_MAJOR=1.
 *
 * The resident tile-major path keeps every transformer block's weights live
 * at once. A weight cache smaller than the full ~9 GiB F32 decoder then
 * re-uploads the entire weight set for every chunk/tile state. This
 * traversal first prepares only each state's hidden tensor, then streams
 * every block through the weight cache exactly once and applies it to every
 * state. The per-state arithmetic is unchanged, so results match the
 * tile-major path bit-for-bit, while weight traffic collapses from
 * states x decoder-size to one decoder scan plus a small hidden pool. */

static int layer_major_enabled(void) {
    const char *value = getenv("H3_VAE_LAYER_MAJOR");
    return value && value[0] == '1';
}

static int pack_layer_major_state(vae_context *vae, h3_gpu_tensor *zero,
                                  char *error, size_t error_size) {
    /* Input projection for one state; the caller points vae->hidden at the
     * state's pool tensor so the packed hidden lands there directly. */
#define OP(call, label) do {                                                    \
    if (!gpu_op(vae, (call), error, error_size, label)) return 0;               \
} while (0)
    OP(h3_gpu_begin(vae->gpu), "begin layer-major video VAE state packing");
    OP(h3_gpu_linear_f32(vae->gpu, vae->post, vae->latent, vae->post_w,
        vae->post_b, vae->patches, LATENT_CHANNELS, LATENT_CHANNELS),
       "layer-major video VAE post-quant projection");
    OP(h3_gpu_linear_f32(vae->gpu, vae->patch_hidden, vae->post, vae->embed_w,
        vae->embed_b, vae->patches, LATENT_CHANNELS, HIDDEN),
       "layer-major video VAE latent embedding");
    OP(h3_gpu_copy_f32(vae->gpu, vae->hidden, 0, vae->patch_hidden, 0,
        (size_t)vae->patches * HIDDEN), "pack layer-major video VAE patches");
    OP(h3_gpu_copy_f32(vae->gpu, vae->hidden,
        (size_t)vae->patches * HIDDEN, vae->registers, 0,
        REGISTERS * HIDDEN), "pack layer-major video VAE registers");
    OP(h3_gpu_copy_f32(vae->gpu, vae->hidden,
        (size_t)(vae->patches + REGISTERS) * HIDDEN, zero, 0, HIDDEN),
       "pack layer-major video VAE suffix");
    OP(h3_gpu_submit(vae->gpu), "submit layer-major video VAE state packing");
#undef OP
    return 1;
}

static int run_layer_major_blocks(vae_context *vae, h3_gpu_tensor **pool,
                                  int states, char *error, size_t error_size) {
    h3_gpu_tensor *shared_hidden = vae->hidden;
    for (int index = 0; index < LAYERS; index++) {
        if (!gpu_op(vae, h3_gpu_begin(vae->gpu), error, error_size,
                    "begin layer-major video VAE transformer block")) {
            vae->hidden = shared_hidden;
            return 0;
        }
        for (int state = 0; state < states; state++) {
            vae->hidden = pool[state];
            if (!run_block(vae, index, error, error_size)) {
                vae->hidden = shared_hidden;
                return 0;
            }
        ...
        if (!gpu_op(vae, h3_gpu_submit(vae->gpu), error, error_size,
                    "submit layer-major video VAE transformer block"))
            return 0;
    }
    return 1;
}

static int project_layer_major_state(vae_context *vae,
                                     h3_gpu_tensor *state_hidden,
                                     char *error, size_t error_size) {
    h3_gpu_tensor *shared_hidden = vae->hidden;
    vae->hidden = state_hidden;
#define OP(call, label) do {                                                    \
    if (!gpu_op(vae, (call), error, error_size, label)) {                       \
        vae->hidden = shared_hidden; return 0;                                  \
    }                                                                           \
} while (0)
    OP(h3_gpu_begin(vae->gpu),
       "begin layer-major video VAE output projection");
    OP(h3_gpu_layer_norm_f32(vae->gpu, vae->norm, vae->hidden,
        vae->norm_out_w, vae->norm_out_b, vae->sequence, HIDDEN, 1e-5f),
       "layer-major video VAE output LayerNorm");
    OP(h3_gpu_linear_f32(vae->gpu, vae->projected, vae->norm,
        vae->proj_w, vae->proj_b, vae->sequence, HIDDEN, OUTPUT_PATCH),
       "layer-major video VAE output projection");
    OP(h3_gpu_submit(vae->gpu),
       "submit layer-major video VAE output projection");
#undef OP
    vae->hidden = shared_hidden;
    return 1;
}
...
static int decode_chunks_layer_major(vae_context *vae,
                        ...
                        int full_latent_h, int full_latent_w,
                        const float *latent_mean, const float *latent_std,
                        int chunk_begin, int chunk_end, int selected_frame,
                        h3_video_frames *output, char *error,
                        size_t error_size) {
    if (output) memset(output, 0, sizeof(*output));
    if (selected_frame >= 0 && chunk_end - chunk_begin != 1) {
        fail(error, error_size,
             "layer-major video VAE preview must decode exactly one chunk");
        return 0;
    }
    if (chunk_begin < 0 || chunk_end <= chunk_begin) {
        fail(error, error_size, "invalid layer-major video VAE chunk range");
        return 0;
    }
    int chunk_count = chunk_end - chunk_begin;
    int tile_count = y_axis->count * x_axis->count;
    int states = chunk_count * tile_count;
    if (states < 1 || tile_count < 1) {
        fail(error, error_size, "invalid layer-major video VAE state plan");
        return 0;
    }
    size_t hidden_elements = (size_t)vae->sequence * HIDDEN;
    if (getenv("H3_PROFILE"))
        fprintf(stderr,
                "h3: layer-major video VAE %d chunks x %d tiles (%d states, "
                "%zu KiB hidden each)\n", chunk_count, tile_count, states,
                hidden_elements * sizeof(float) / 1024);

    float zeros[HIDDEN];
    memset(zeros, 0, sizeof(zeros));
    h3_gpu_tensor *zero = h3_gpu_tensor_from_f32(vae->gpu, zeros, HIDDEN);
    h3_gpu_tensor **pool = calloc((size_t)states, sizeof(*pool));
    if (!zero || !pool) {
        h3_gpu_tensor_free(zero);
        free(pool);
        fail(error, error_size,
             "out of memory preparing layer-major video VAE state pool");
        return 0;
    }

    int ok = 1;
    /* Phase A: input projection only, one persistent hidden per tile/chunk. */
    for (int chunk = chunk_begin; chunk < chunk_end && ok; chunk++)
        for (int tile_y = 0; tile_y < y_axis->count && ok; tile_y++)
            for (int tile_x = 0; tile_x < x_axis->count && ok; tile_x++) {
                int state = (chunk - chunk_begin) * tile_count +
                            tile_y * x_axis->count + tile_x;
                pool[state] = h3_gpu_tensor_new_f32(vae->gpu, hidden_elements);
                if (!pool[state]) {
                    fail(error, error_size,
                         "cannot allocate the layer-major video VAE hidden "
                         "pool (%d states of %zu bytes): %s; unset "
                         "H3_VAE_LAYER_MAJOR to use tile-major decoding",
                         states, hidden_elements * sizeof(float),
                         h3_gpu_error(vae->gpu));
                    ok = 0;
                    break;
                }
                float *input = extract_latent_tile(
                    normalized_latent, latent_time, full_latent_h,
                    full_latent_w, chunk * 5,
                ...
                ok = prepare_input(vae, input, latent_mean, latent_std,
                                   error, error_size) &&
                     pack_layer_major_state(vae, zero, error, error_size);
                vae->hidden = shared_hidden;
                free(input);
            }

    /* Phase B: each block's weights stream through the cache exactly once. */
    if (ok) ok = run_layer_major_blocks(vae, pool, states, error, error_size);

    /* Phase C: output projection, unpack, stitch and blend per chunk. */
    int pixel_h = full_latent_h * SPATIAL_RATIO;
    int pixel_w = full_latent_w * SPATIAL_RATIO;
    size_t frame_elements = (size_t)pixel_h * (size_t)pixel_w * 3;
    float *final_rgb = NULL;
    float *temporal_overlap = NULL;
    if (ok && selected_frame < 0) {
        int output_frames = chunk_count * 17 + 5;
        size_t output_elements = (size_t)output_frames * frame_elements;
        final_rgb = malloc(output_elements * sizeof(*final_rgb));
        temporal_overlap = malloc(5 * frame_elements *
                                  sizeof(*temporal_overlap));
        if (!final_rgb || !temporal_overlap) {
            fail(error, error_size,
                 "out of memory retaining layer-major decoded video frames");
            ok = 0;
        }
    }
    ...
    for (int chunk = chunk_begin; chunk < chunk_end && ok; chunk++) {
        for (int tile_y = 0; tile_y < y_axis->count && ok; tile_y++)
            for (int tile_x = 0; tile_x < x_axis->count && ok; tile_x++) {
                ...
                if (!project_layer_major_state(vae, pool[state], error,
                                               error_size)) {
                    ok = 0;
                    break;
                }
                ...
                if (ok) tiles[index] = tile.rgb;
            }
        h3_video_frames decoded;
        memset(&decoded, 0, sizeof(decoded));
        int stitched_frames = selected_frame >= 0 ? 1 : FIRST_CHUNK_FRAMES;
        if (ok) ok = stitch_tiles(tiles, y_axis, x_axis, stitched_frames,
        ...
        if (chunk) for (int frame = 0; frame < 5; frame++) {
            ...
            for (size_t element = 0; element < frame_elements; element++)
                decoded.rgb[base + element] =
                    temporal_overlap[base + element] * (1.0f - alpha) +
                    decoded.rgb[base + element] * alpha;
        }
    ...
    if (ok && selected_frame < 0) {
        memcpy(final_rgb + (size_t)chunk_count * 17 * frame_elements,
               temporal_overlap, 5 * frame_elements * sizeof(*final_rgb));
        output->frames = chunk_count * 17 + 5;
    ...
    free(tiles);
    h3_gpu_tensor_free(zero);
    for (int state = 0; state < states; state++) free_tensor(&pool[state]);
    free(pool);
...
int h3_video_vae_decoder_preview(h3_video_vae_decoder *decoder,
    ...
    int ok = layer_major_enabled() ?
        decode_chunks_layer_major(&decoder->vae, &decoder->y_axis,
                                  &decoder->x_axis, normalized_latent,
                                  latent_time, decoder->latent_h,
                                  decoder->latent_w, decoder->latent_mean,
                                  decoder->latent_std, chunk, chunk + 1,
                                  local_frame, output, error, error_size) :
        decoder_decode_chunk(decoder, normalized_latent, latent_time,
...
int h3_video_vae_decoder_decode(h3_video_vae_decoder *decoder,
    ...
    int chunks = (latent_time - 2) / 5;
    if (layer_major_enabled())
        return decode_chunks_layer_major(&decoder->vae, &decoder->y_axis,
            &decoder->x_axis, normalized_latent, latent_time,
            decoder->latent_h, decoder->latent_w, decoder->latent_mean,
            decoder->latent_std, 0, chunks, -1, output, error, error_size);
    int output_frames = chunks * 17 + 5;
...
static int decode_chunked(const char *weight_directory,
    ...
    ok = vae.weights && vae.gpu &&
         ...
         prepare_rope(&vae, error, error_size) &&
         allocate_activations(&vae, error, error_size);
    if (ok && layer_major_enabled()) {
        int layer_major_chunks = (latent_time - 2) / 5;
        int done = decode_chunks_layer_major(&vae, &y_axis, &x_axis,
            normalized_latent, latent_time, latent_height, latent_width,
            latent_mean, latent_std, 0, layer_major_chunks, -1, output,
            error, error_size);
        cleanup(&vae);
        tile_axis_free(&y_axis); tile_axis_free(&x_axis);
        return done;
