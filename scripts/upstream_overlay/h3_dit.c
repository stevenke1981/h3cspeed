...
#define LOAD_I8(field, scales, suffix, rows, columns) do {                    \
    uint64_t shape[] = {rows, columns};                                        \
    snprintf(name, sizeof(name), "%s%s", prefix, suffix);                    \
    block->field = h3_weight_load_i8_convrot(                                  \
        dit->weights, dit->gpu, name, 2, shape, &block->scales,                \
        error, error_size);                                                    \
    if (!block->field || !block->scales) return 0;                             \
} while (0)
    LOAD1(norm1, "norm1.weight", HIDDEN);
    LOAD1(norm2, "norm2.weight", HIDDEN);
    LOAD_I8(qkv_int8, qkv_scales, "attn.qkv_proj.weight", INNER * 3, HIDDEN);
    LOAD1(q_norm, "attn.q_norm.weight", HEAD_DIM);
    LOAD1(k_norm, "attn.k_norm.weight", HEAD_DIM);
    LOAD_I8(out_int8, out_scales, "attn.out_proj.weight", HIDDEN, INNER);
    LOAD_I8(fc1_int8, fc1_scales, "mlp.fc1.weight", FFN * 2, HIDDEN);
    LOAD_I8(fc2_int8, fc2_scales, "mlp.fc2.weight", HIDDEN, FFN);
#undef LOAD1
#undef LOAD_I8
    return 1;
}
...
#define OP(call, label) do {                                                    \
    ...
    /* The Comfy quantized checkpoint stores contiguous [Q,K,V] projection
     * rows, while the pinned upstream checkpoint stores per-head groups. */
    OP(dit->checkpoint_convrot_int8 ? h3_gpu_qkv_rope_bf16(
           dit->gpu, query, key, value, qkv, weight->q_norm, weight->k_norm,
           weight->q_norm, weight->q_norm, rows, HEADS, HEAD_DIM, 0, 1e-5f) :
...
static int load_core(h3_dit *dit, h3_dit_progress progress, void *opaque,
    ...
    for (unsigned index = 0; index < H3_DIT_BLOCKS; index++) {
        ...
        if (dit->checkpoint_convrot_int8) {
...
static h3_dit *load_dit(const char *weight_directory,
    ...
    dit->checkpoint_convrot_int8 = h3_weight_is_convrot_i8(
        dit->weights, "blocks.0.attn.qkv_proj.weight");
    if (dit->checkpoint_convrot_int8 && dit->ssd_streaming) {
        /* Direct INT8 weights are already one quarter the BF16 stream volume.
         * Let the common RAM/file LRU page them instead of expanding them into
         * the two reusable BF16 SSD slots. */
        dit->ssd_streaming = 0;
        if (getenv("H3_PROFILE"))
            fprintf(stderr, "h3: ConvRot INT8 checkpoint uses RAM/file offload\n");
    }
    ...
    dit->int8_mlp = dit->checkpoint_convrot_int8 ||
                    (!dit->ssd_streaming && dit->fused_mlp &&
                    !use_slower_bf16_mlp &&
                    h3_gpu_has_int8_mlp(dit->gpu));
    dit->int8_qkv = dit->checkpoint_convrot_int8 ||
                    (!dit->ssd_streaming && !use_slower_bf16_qkv &&
                    dit->sequence >= 128 &&
                    h3_gpu_has_int8_mlp(dit->gpu));
    dit->int8_attention_out = dit->checkpoint_convrot_int8 ||
                              (!dit->ssd_streaming &&
