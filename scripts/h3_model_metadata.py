...
SUPPORTED_VARIANTS = {"time-embedder", "adaln-curves"}
...
def detect_config(tensors: Mapping[str, TensorInfo], prefix: str) -> ModelConfig:
    ...
    if adaln is not None:
        ...
        if grid < 2 or dimension != 8:
            raise InspectionError(
                f"tensor {adaln.name} is not a valid AdaLN curve table; "
                "expected [grid, 8] with grid >= 2"
            )
        values.update(
...
def compatibility_issues(config: ModelConfig) -> tuple[str, ...]:
    ...
    if config.variant not in SUPPORTED_VARIANTS:
        issues.append(
            f"variant={config.variant} (expected one of {sorted(SUPPORTED_VARIANTS)})"
        )
    for field, expected in EXPECTED.items():
        if field == "variant":
            continue
        if config.variant == "adaln-curves" and field in {
            "timestep_input_dim", "time_embed_hidden_size", "time_embed_dim"
        }:
            continue
        if getattr(config, field) != expected:
            issues.append(f"{field}={getattr(config, field)} (expected {expected})")
    if config.variant == "adaln-curves" and (
        config.adaln_curve_grid < 2 or config.time_embed_dim != 8
    ):
        issues.append(
            "adaln_curve_grid=%d and curve_dim=%d (expected grid >= 2 and dim 8)"
            % (config.adaln_curve_grid, config.time_embed_dim)
        )
    return tuple(issues)
